...
def get_ip_routing_table_temp(host, port, username, password):
    with manager.connect(host=host, port=port, username=username,
        ...
        rpc_command = to_ele(display_ip_routing_table())
        response = m.dispatch(rpc_command)
        print(response.xml)

def get_ip_routing_table():
    get_ip_routing_table_temp(hostname, port, username, password)

# if __name__ == '__main__':
#     get_ip_routing_table(hostname, port, username, password)
